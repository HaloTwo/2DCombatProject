...
짧은 플레이 안에서 이동, 회피, 타격감, 스킬, 패링, 웨이브, 버프, 포커스 모드가 한 번에 보이도록 만든 포트폴리오용 개인 프로젝트입니다.
...
- 오브젝트 풀 기반 이펙트/투사체/드랍 아이템/대쉬 잔상 재사용

## 포트폴리오 등록용 요약

아래 행은 개인 프로젝트 목록 표에 바로 추가할 수 있습니다.
...
<details>
<summary><strong>포트폴리오 상세 문구</strong></summary>
...
- **목표:** 2D 액션 게임의 조작감, 타격감, 반복 전투 루프를 포트폴리오용으로 구현
...
- 비행/근접/원거리처럼 플레이 화면에서 차이가 바로 보이는 부분만 파생 클래스에서 처리했습니다.
...
- 보스전을 별도 시스템으로 과하게 새로 만들기보다, 이미 검증한 적/피격/웨이브 구조를 확장했습니다.
- 마지막 웨이브가 단순 몹 정리로 끝나지 않고 시연 영상에서 클라이맥스처럼 보이게 했습니다.
...
- 시연 영상에서 실패/클리어 UI까지 보여줄 수 있습니다.
...
- 액션 게임에서 "결정적인 순간"을 보여주기 좋아 시연 영상용 임팩트가 큽니다.
...
- 시연 영상에서도 적 처치, 투사체, 이펙트가 여러 번 발생하므로 풀 구조가 필요합니다.
...
<details>
<summary>포트폴리오 시연 영상 기획안</summary>
...
포트폴리오용 전투 프로토타입은 기능 개수보다 "플레이가 되는가"가 중요하다고 판단했습니다.  
...
- 적 AI는 현재 시연용 추적/순찰 중심 구조이며, 더 자연스러운 추적을 위해 간단한 플랫폼 경로 탐색을 추가할 수 있습니다.
