...
namespace SmallScaleInteractive._2DCharacter
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void HandleMovement()
        {
            ...
            bool isPressingLeft = keyboard.aKey.isPressed || keyboard.leftArrowKey.isPressed;
            bool isPressingRight = keyboard.dKey.isPressed || keyboard.rightArrowKey.isPressed;
        ...
        {
            if (mouse != null && mouse.leftButton.wasPressedThisFrame && isGrounded) // Left mouse button for attack
            {
        ...
        private void HandleJumping()
        {
            if (canMove)
            {
                ...
                else if (!isGrounded && !isWallHanging && keyboard.sKey.wasPressedThisFrame) // Check if in air and S key is pressed
                {
                    StartAirSlam();
                }
            }
        ...
        void HandleAirDash()
        {
            if (mouse != null && mouse.rightButton.wasPressedThisFrame && !isGrounded && !isWallHanging)  // Right mouse button for air dash
            {
        ...
        {
            if (mouse != null && mouse.rightButton.wasPressedThisFrame && isGrounded)  // Right mouse button for ground dash
            {
                float horizontalInput = ReadHorizontalInput();
        ...
        {
            if (mouse != null && mouse.rightButton.wasPressedThisFrame && isGrounded && Time.time >= lastDashTime + dashCooldown)  // Right mouse button for ground dash and check cooldown
            {
        ...
        void FixedUpdate()
        {
            ...
            {
                float moveVertical = ReadVerticalInput();
        ...
        void HandleSliding()
        {
            if (keyboard.leftShiftKey.wasPressedThisFrame && isGrounded && !isCrouching)
            {
        ...
        {
            if (keyboard.digit1Key.wasPressedThisFrame && isGrounded) // Check if '1' key is pressed
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
        ...
        {
            if (keyboard.digit2Key.wasPressedThisFrame && isGrounded) // Check if '2' key is pressed
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
        ...
        {
            if (keyboard.fKey.wasPressedThisFrame && isGrounded) // Check if 'F' key is pressed
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
        ...
        {
            if (keyboard.vKey.wasPressedThisFrame && isGrounded) // Check if 'V' key is pressed
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
